...
def model(Ws, inputs):
    ...
    b1 = W1[-1]
    x1 = tf.tanh( tf.nn.conv2d(inputs, w1, [1,1,1,1], 'VALID') + b1 )
        
    w2 = tf.reshape(W2[:-1], [3, 3, num_f, num_f])
    b2 = W2[-1]
    x2 = tf.tanh( tf.nn.conv2d(x1, w2, [1,1,1,1], 'VALID') + b2 )
    
    x2 = tf.nn.avg_pool(x2, [1,2,2,1], [1,2,2,1], 'VALID')
    ...
    x4 = tf.nn.avg_pool(x4, [1,2,2,1], [1,2,2,1], 'VALID')
